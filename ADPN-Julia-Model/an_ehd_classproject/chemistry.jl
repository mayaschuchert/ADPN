...
Fill R::AbstractVector (length ≥ 9) with buffer rates [mol m⁻³ s⁻¹]:
indices 1..5 are H⁺, OH⁻, H₂PO₄⁻, HPO₄²⁻, PO₄³⁻; 6..9 (AN/ADPN/PN/TCH) set to 0.
...
@inline function bulk_concentration(k::Int, c_eq, eps_org::Float64)
    ...
    k == 7 && return 1.0e-3
    k == 8 && return 1.0e-3
    k == 9 && return 1.0e-3     # TCH trace seed (product, zero at bulk)
...
Build 10·N vector of DOFs (cell-major): 9 log-concentrations + φ_l per cell.
...
function make_initial_guess(N_mesh::Int, c_eq, eps_org::Float64;
    ...
    log_c0 = [log(c_eq.H),    log(c_eq.OH),
              ...
              log(c_seed),     # PN seed
              log(c_seed)]     # TCH seed
    @assert length(log_c0) == 9
    for ix in 1:N_mesh
        base = 10 * (ix - 1)
        @inbounds for k in 1:9
            u0[base + k] = log_c0[k]
        end
        u0[base + 10] = 0.0   # φ_l = 0 everywhere
    end
    return u0
end
