"""
plot_residuals.py — residual diagnostic plots for the published kinetics fit.

Inputs:
  output/data/parity.csv

Outputs (6 plots in output/plots/):
...
DATA_FN  = os.path.join(HERE, "output", "data", "parity.csv")
...
def faceted_residual(df, x_col, resid_col, x_label, resid_label, fname, ymax=None):
    ...
    fig.suptitle(
        f"Published fit — {resid_label} vs {x_label}",
        y=1.06, fontsize=12,
    )
    fig.tight_layout()
...
def main():
    if not os.path.exists(DATA_FN):
        ...
        print("Run publish_fit.jl first.")
        return
