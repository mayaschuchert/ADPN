"""
plot_weber_maps.py — 3D GPR cube maps over the MODEL's FE_ADN predictions.

Matches Bloomquist Fig. 5 style: three solid-colored cube faces showing how
the published-fit model interpolates FE_ADN over flow-regime space
(We_aq × We_org × j) at each gap.

The GPR is fit to `FE_ADN_pred` from the model (output/data/parity.csv), not
to experimental observations, so this visualizes the model's behaviour across
the design space rather than the raw data.

Input:  output/data/parity.csv      (model predictions written by publish_fit.jl)
Output: output/plots/weber_maps.png
"""
import os
import numpy as np
import pandas as pd
...
PARITY_CSV = os.path.join(HERE, "output", "data", "parity.csv")
...
def main():
    ...
    # Keep only converged rows where the model produced a prediction
    ...
    # Tighten color range to the actual model FE_ADN distribution (~35-72 %)
    # rather than the [0, 80] full-experimental range — the model's dynamic
    # range is genuinely narrower than experiments because n_ADN=0.629 gives
    # weak AN-dependence (model can't reproduce FE_ADN→0 at low AN loadings).
    fe_min, fe_max = df["FE_ADN_pred"].min(), df["FE_ADN_pred"].max()
    pad = 0.05 * (fe_max - fe_min)
    cmap = plt.cm.turbo
    norm = Normalize(vmin=fe_min - pad, vmax=fe_max + pad)
    print(f"Color range: FE_ADN_pred in [{fe_min:.1f}, {fe_max:.1f}] %  "
          f"(plotted with +/-{pad:.1f} pp padding)")
    ...
    for k, (gap, glabel) in enumerate(zip(GAP_ORDER, GAP_LABELS)):
        ...
        fe = gdf["FE_ADN_pred"].values   # MODEL predictions (not experimental)
    ...
    # ── Layout: reserve right margin for the colorbar so it doesn't overlap ──
    ...
    fig.suptitle(
        "Weber-number regime map  (model predictions)\n"
        "GPR surface over modeled FE$_{\\mathrm{ADN}}$ as a function of "
        "We$_{\\mathrm{aq}}$ × We$_{\\mathrm{org}}$ × j",
        fontsize=15, y=1.00,
    )
