# -----------------------------------------------------------------------------
# run_stage4v3_cont.jl — v3 fit pipeline driven by the new continuation solver.
#
# Same fitting algorithm as run_stage4v3.jl (joint 9-param LM, HER frozen,
# ADN/PN/TCH fitted), but routes residual evaluations through
# GalvContinuation.solve_at_j_cont instead of FixedJ.solve_at_j.
#
# Outputs go to output/stage4v3_cont/ to keep the original v3 results clean.
# -----------------------------------------------------------------------------
using Printf
using Dates
using LinearAlgebra
...
using .ADPN_EHD.GalvContinuation       # ensure module is loaded
...
const OUT_DIR      = joinpath(@__DIR__, "output", "stage4v3_cont")
...
function main()
    println("=" ^ 72)
    println(" Stage 4v3-cont — v3 joint LM driven by galv-continuation solver")
    println(" $(now())")
    println("=" ^ 72)
    ...
    @printf("LB: %s\nUB: %s\n", THETA_LB, THETA_UB)
    @printf("Solver backend: %s\n", FitKinetics.SOLVER_BACKEND[])
    ...
    open(joinpath(OUT_DATA_DIR, "stage4a_fitted_theta.txt"), "w") do io
        @printf(io, "# v7 Stage 4v3-cont — joint LM via galv-continuation solver\n")
        @printf(io, "# date: %s\n", now())
