"""
plot_stage4_residuals.py — residual diagnostic plots for the v7 Stage 4v3 fit.

Inputs (output/stage4v3/data/):
  stage4a_core_residuals.csv
  stage4b_extended_residuals.csv
  stage4b_holdout_residuals.csv

Outputs (output/stage4v3/plots/):
  stage4v3_resid_FE_ADN_vs_j.png
  stage4v3_resid_FE_ADN_vs_eps.png
  stage4v3_resid_FE_PN_vs_j.png
  stage4v3_resid_FE_PN_vs_eps.png
  stage4v3_resid_FE_TCH_vs_j.png
  stage4v3_resid_FE_TCH_vs_eps.png
"""

import os
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

HERE     = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(HERE, "output", "stage4v3", "data")
PLOT_DIR = os.path.join(HERE, "output", "stage4v3", "plots")
os.makedirs(PLOT_DIR, exist_ok=True)

GAP_ORDER = (0.25, 0.5, 1.0)
SUBSET_COLOR = {
    "Core":     "#1f77b4",
    "Extended": "#ff7f0e",
    "Holdout":  "#d62728",
}
SUBSET_MARKER = {
    "Core":     "o",
    "Extended": "s",
    "Holdout":  "^",
}


def load_data():
    frames = []
    for fname, tag in [
        ("stage4a_core_residuals.csv",     "Core"),
        ("stage4b_extended_residuals.csv", "Extended"),
        ("stage4b_holdout_residuals.csv",  "Holdout"),
    ]:
        df = pd.read_csv(os.path.join(DATA_DIR, fname))
        df["subset"] = tag
        frames.append(df)
    return pd.concat(frames, ignore_index=True)


def faceted_residual(df, x_col, resid_col, x_label, resid_label, fname, *,
                     ymax=None):
    fig, axes = plt.subplots(1, 3, figsize=(14, 4.5), sharey=True)
    sub = df[np.isfinite(df[x_col]) & np.isfinite(df[resid_col])].copy()

    for ax, gap in zip(axes, GAP_ORDER):
        gdf = sub[sub["gap_mm"] == gap]
        for tag, sdf in gdf.groupby("subset"):
            ax.scatter(sdf[x_col], sdf[resid_col],
                       c=SUBSET_COLOR.get(tag, "k"),
                       marker=SUBSET_MARKER.get(tag, "o"),
                       s=30, alpha=0.8, edgecolors="none",
                       label=tag if ax is axes[0] else None)
        ax.axhline(0, color="k", lw=1, alpha=0.5)
        ax.set_title(f"gap = {gap} mm  (n = {len(gdf)})")
        ax.set_xlabel(x_label)
        if ax is axes[0]:
            ax.set_ylabel(resid_label)
        annot_lines = []
        for tag in ("Core", "Extended", "Holdout"):
            sdf = gdf[gdf["subset"] == tag]
            if len(sdf) == 0:
                continue
            r = np.sqrt(np.mean(sdf[resid_col].values ** 2))
            annot_lines.append(f"{tag}: RMSE = {r:.2f}  (n={len(sdf)})")
        if annot_lines:
            ax.text(0.04, 0.96, "\n".join(annot_lines),
                    transform=ax.transAxes, ha="left", va="top", fontsize=8,
                    bbox=dict(facecolor="white", edgecolor="0.8",
                              alpha=0.9, pad=4))
        if ymax is not None:
            ax.set_ylim(-ymax, ymax)
        ax.grid(alpha=0.3)

    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center",
               ncol=3, fontsize=9, bbox_to_anchor=(0.5, 1.02), frameon=False)
    fig.suptitle(f"Stage 4v3 — {resid_label} vs {x_label}",
                 y=1.06, fontsize=12)
    fig.tight_layout()
    path = os.path.join(PLOT_DIR, fname)
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"wrote {path}")


def main():
    df = load_data()
    print(f"loaded {len(df)} rows")

    faceted_residual(df, "j_mA_cm2", "FE_ADN_resid_pp",
                     "j [mA/cm²]", "FE_ADN residual [pp]",
                     "stage4v3_resid_FE_ADN_vs_j.png", ymax=50)
    faceted_residual(df, "phi_AN", "FE_ADN_resid_pp",
                     "ε_org (= φ_AN)", "FE_ADN residual [pp]",
                     "stage4v3_resid_FE_ADN_vs_eps.png", ymax=50)

    faceted_residual(df, "j_mA_cm2", "FE_PN_resid_pp",
                     "j [mA/cm²]", "FE_PN residual [pp]",
                     "stage4v3_resid_FE_PN_vs_j.png", ymax=20)
    faceted_residual(df, "phi_AN", "FE_PN_resid_pp",
                     "ε_org (= φ_AN)", "FE_PN residual [pp]",
                     "stage4v3_resid_FE_PN_vs_eps.png", ymax=20)

    faceted_residual(df, "j_mA_cm2", "FE_TCH_resid_pp",
                     "j [mA/cm²]", "FE_TCH residual [pp]",
                     "stage4v3_resid_FE_TCH_vs_j.png", ymax=10)
    faceted_residual(df, "phi_AN", "FE_TCH_resid_pp",
                     "ε_org (= φ_AN)", "FE_TCH residual [pp]",
                     "stage4v3_resid_FE_TCH_vs_eps.png", ymax=10)


if __name__ == "__main__":
    main()
