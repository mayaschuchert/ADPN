...
# and uses the historical fixed AN reaction orders n_ADN=2, n_PN=1; all Stage
...
# (n_ADN for ADPN, n_PN for PN) to fit parameters. Tuple ordering is
# (n_ADN, n_PN). Backwards-compat: callers that still pass only (j0, ac)
# get the historical defaults n=(2.0, 1.0).
