# run_stage4_seq_cont.jl — Same algorithm as run_stage4_seq.jl
# (Sequential decoupled kinetics fit, Bloomquist SI §S1.11) but routed through
# the new GalvContinuation solver on post-kink-fix physics. Output redirected
# to output/stage4_seq_cont/ so the preserved pre-fix baseline at
# output/preserved_pre_kink_fix/ stays clean.
#
...
using .ADPN_EHD.GalvContinuation       # ensure module loaded

# ---- Switch backend BEFORE building any context ----
FitKinetics.SOLVER_BACKEND[] = :cont
@info "Solver backend set to" FitKinetics.SOLVER_BACKEND[]

# ---- Adapter so the existing seq_residuals!/lm3 code can call into
#      GalvContinuation.solve_at_j_cont with the same arg/kwarg shape as
#      FixedJ.solve_at_j.  Drops V_hi/V_lo/ds_min/max_walk/max_bisect (V-walk
#      knobs that don't apply to the continuation solver).
function solve_at_j(j_target, eps_org, _delta_m, mesh, u_warm, c_eq;
                    j0 = nothing, alpha_c = nothing, n_orders = nothing,
                    V_hint::Float64 = NaN,
                    u_hint::Union{Nothing,Vector{Float64}} = nothing,
                    V_hi::Float64 = -0.8,           # accepted, ignored
                    V_lo::Float64 = -2.5,           # accepted, ignored
                    ds_init::Float64 = 0.05,        # accepted, ignored
                    ds_min::Float64 = 5.0e-4,       # accepted, ignored
                    ds_max::Float64 = 0.20,         # accepted, ignored
                    tol_rel::Float64 = 1.0e-3,
                    max_walk::Int = 200,            # accepted, ignored
                    max_bisect::Int = 40,           # accepted, ignored
                    jacobian_mode::Symbol = :fd,    # accepted, ignored
                    newton_max_iter::Int = 60,
                    verbose::Bool = false)
    warm_j = (!isnan(V_hint) && u_hint !== nothing) ? j_target : NaN
    return solve_at_j_cont(j_target, eps_org, mesh, c_eq;
                           j0 = j0, alpha_c = alpha_c, n_orders = n_orders,
                           warm_u = u_hint,
                           warm_V = V_hint,
                           warm_j = warm_j,
                           tol_rel_j = tol_rel,
...
const OUT_DIR      = joinpath(@__DIR__, "output", "stage4_seq_cont")
