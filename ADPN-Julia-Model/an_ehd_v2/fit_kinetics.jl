...
#            n_ADN, n_PN)
#
# v6.x: Promoted the AN reaction orders n_ADN (ADPN) and n_PN (PN) to fit
# parameters per Finding 1 of the v6 first-fit changelog. The previous v6 fit
# (now in output/stage4/) ran with n_ADN, n_PN fixed at (2, 1).
#
# Row filters (§20.1):
...
# n_ADN ∈ [1, 3] — ADPN reaction order in c_AN (Mathison JACS 2025 et al.).
# n_PN  ∈ [0.5, 2] — PN reaction order; floor at 0.5 keeps c_AN^n well-defined.
