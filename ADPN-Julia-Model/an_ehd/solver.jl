...
# ------------------------------------------------------------
# Build sparsity pattern: block-tridiagonal, block size 9.
# Cell-major DOF layout ⇒ coupling of cell ix to cells ix−1, ix, ix+1
# ⇒ half-bandwidth b = 2·blocksize − 1 = 17.
# Column-grouped FD requires 2b+1 = 35 perturbation vectors.
# ------------------------------------------------------------
const JAC_BLOCK  = 9
...
function newton_solve!(u::Vector{Float64}, residual!::F;
    ...
    for iter in 1:max_iter
        ...
        @inbounds for ix in 1:N_cells
            ...
            du[base + 9] = clamp(du[base + 9], -0.015, 0.015)
        end
    ...
    for step in 1:N_steps
        ...
        for iter in 1:max_iter
            ...
            @inbounds for ix in 1:N_cells
                ...
                dw[base + 9] = clamp(dw[base + 9], -0.015, 0.015)
            end
